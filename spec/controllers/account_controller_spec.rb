...
describe AccountController do
  describe "before authentication" do
    ...
    it "should redirect after successful login (no test plans)" do
      controller.stub!(:current_user).and_return mock_model(User, :count_test_plans => 0)
      post :login, { :email => 'foo@bar.com', :password => 'barfoo' }
      response.should redirect_to(pophealth_index_url)
    end

    it "should redirect after successful login (with test plans)" do
    ...
    it "should redirect after successful signup" do
      post :signup, {
        :user => {
          :first_name => 'bar',
          :last_name => 'foo',
